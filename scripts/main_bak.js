...
(() => {
  ...
  if (IS_RELEASE_BUILD) {
    ...
    [flagZpad, flagComboRed, flagChainDelay, flagSlowTargets,
     ui.glow, ui.trail, ui.auto, ui.power, ui.gravity, ui.particles].forEach(hideWithWrap);
  }
...
async function getJSON(url){
  ...
  // Fever Time settings
  const FEVER_REQUIRE_DURATION = 10.0; // MAX状態が続いた秒数
  const FEVER_DURATION = 10.0;         // フィーバータイム持続時間
  let feverActive = false;
  let feverTriggered = false;
  let feverStartAt = 0;
  let comboMaxStartAt = null; // MAXコンボに到達した時間

  // Fever visual (canvas backfill: black ↔ vivid yellow, ping-pong fade)
  const FEVER_FLASH_PERIOD = 0.9;                 // 秒（0→1→0 の往復周期）
  const FEVER_YELLOW = { r:255, g:225, b:0 };     // ビビッド黄
  function pingpong01(t, period){                 // 0→1→0 の線形往復
    const m = (t % period) / period;              // 0..1 saw
  ...
  async function showResult(){
    ...
    // Ensure current run is present, then sort & clamp (for display)
    const nowT = Date.now();
  ...
  function updateHUD(){
    ...
    if (soloUI.combo) {
      ...
      // Fever trigger candidate tracking
      if (displayCombo >= MAX_COMBO) {
        if (comboMaxStartAt == null) comboMaxStartAt = performance.now()/1000;
      } else {
        comboMaxStartAt = null;
      }
    }
  ...
  function stepTargets(dt){
    ...
    // 既存の slowTargets フラグを尊重しつつ、終盤ブースト + フィーバー2x
    const baseSdt = flags().slowTargets ? dt / 3 : dt;
    const sdt = baseSdt * timeVelMult * (feverActive ? 2.0 : 1.0);
    ...
    const targetEvery = lerp(START_SPAWN, BASE_SPAWN / 8, progress) / (feverActive ? 2.0 : 1.0); // フィーバー中は2x頻度
...
function onHit(ix){
  ...
  function loop(){
    ...
    // 1) 背景クリア（フィーバー時は黒↔黄のピンポン塗り）
    if (feverActive) {
      const k = pingpong01(performance.now()/1000, FEVER_FLASH_PERIOD); // 0..1..0
      const r = Math.round(FEVER_YELLOW.r * k);
      const g = Math.round(FEVER_YELLOW.g * k);
      ctx.fillStyle = `rgb(${r},${g},0)`;
    } else {
      ctx.fillStyle = '#0f172a'; // bg-slate-900 相当
    }
    ...
    // ---- Fever trigger and state management ----
    const nowSec = performance.now()/1000;
